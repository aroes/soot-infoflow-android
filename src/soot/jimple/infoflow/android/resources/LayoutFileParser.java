...
public class LayoutFileParser extends AbstractResourceParser {
	 ...
	 */
	public void parseLayoutFile(final String fileName, final String fileName2) {
		...
		if (fileName2 != null) {
			Transform transform2 = new Transform("wjtp.lfp2", new SceneTransformer() {
				protected void internalTransform(String phaseName, @SuppressWarnings("rawtypes") Map options) {
					parseLayoutFileDirect(fileName2);
				}
			...
			public void handleResourceFile(final String fileName, Set<String> fileNameFilter, InputStream stream) {
				...
				try {
					...
					if (!userControls.isEmpty())
						System.out.println("Found " + userControls.size() + " layout controls in file " + fileName);
				} catch (Exception ex) {
	...
	private void parseLayoutNode(String layoutFile, AXmlNode rootNode) {
		if (rootNode.getTag() == null || rootNode.getTag().isEmpty()) {
			System.err
					.println("Encountered a null or empty node name " + "in file " + layoutFile + ", skipping node...");
			return;
		}
	...
	private void parseLayoutAttributes(String layoutFile, SootClass layoutClass, AXmlNode rootNode) {
		...
		int id = -1;
		Map<String, Object> additionalAttributes = loadAdditionalAttributes ? new HashMap<String, Object>() : null;
		...
		if (!loadOnlySensitiveControls || isSensitive)
			this.userControls.put(layoutFile, new LayoutControl(id, layoutClass, isSensitive, additionalAttributes));
	}
	 ...
	 * password fields
	 * 
	 * @param loadOnlySensitiveControls
	 *            True if this analysis shall only collect sensitive controls,
	 *            otherwise false
	 */
	 ...
	 *            True to load all attributes present in the layout XML files, false
	 *            to only load the ones required for FlowDroid.
	 */
